...
- **`resonate-mpris` sees the engine only through `Player` and the front end only through `Host`.**
  ...
  by edit wherever it can be — `tracklist::change` keys the rows by id, holds still the longest
  run of the rows both samples hold that is still in the same order, and announces every other
  row that left *or moved* as a `TrackRemoved` and every row that arrived or moved as a
  `TrackAdded`, in the order that leaves every `AfterTrack` naming a row already announced — so
  two rows queued inside one poll reach a client as two additions, and a row dragged to the
  front as that row removed and added again, rather than as a list replaced. A reorder that
  moves more rows than it leaves standing — a shuffle — a list with no row in common with the
  last and more than `EDITS_ANNOUNCED_AT_MOST` edits are `TrackListReplaced`, and the additions
  of one sample share one `ANNOUNCE_BUDGET` for their tags rather than a budget each.
  `two_rows_added_between_two_polls_are_announced_as_two_additions` and
  `a_row_moved_is_announced_as_that_row_removed_and_added_again` are the claims. `Tracks` is declared `invalidates`, as the spec
